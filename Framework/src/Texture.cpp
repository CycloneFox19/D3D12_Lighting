...
D3D12_SHADER_RESOURCE_VIEW_DESC Texture::GetViewDesc(bool isCube)
{
	...
	viewDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING; // set the return value of viewDesc

	switch (desc.Dimension) // means the same as if statement contains multiple if statement.
	{
		case D3D12_RESOURCE_DIMENSION_BUFFER:
		{
			abort();
		}
		break; // stops execution of the following code.
		case D3D12_RESOURCE_DIMENSION_TEXTURE1D:
		{
			if (desc.DepthOrArraySize > 1)
			{
				...
				viewDesc.Texture1DArray.FirstArraySlice = 0; // just the first factor to be used.
		...
		case D3D12_RESOURCE_DIMENSION_TEXTURE2D:
		{
			if (isCube)
			{
				if (desc.DepthOrArraySize > 6)
				{
					...
					viewDesc.TextureCubeArray.First2DArrayFace = 0; // index to the first 2D Texture to use
			...
			else
			{
				...
				else
				{
					if (desc.MipLevels > 1)
					{
						viewDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DMS; // if the texture is ms, then the index of pixels can be specified by integer value such as 1 and 7(depending on the size of texture)
					}
					else
					{
